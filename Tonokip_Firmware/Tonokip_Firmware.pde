...
long timediff=0;
int z_feedrate = 20; // constant for z feedrate, no need to use limit in Skeinforge anymore
...
void loop()
{
 ...
 // Serial.print(">>  ");
 //Serial.println (analog2temp(current_raw_in));
  //Serial.print("oC=Raw_in  ");
 //Serial.println(current_raw);
  //Serial.print(">>  ");
  //Serial.println (tt);
  //Serial.println("oC=Raw_avg  ");
...
*/
  ...
  int cutOffB = cutOff;
  int cutOffT = 100 - cutOff;
  
  bottom = max(((filterSamples * cutOffB)  / 100), 1); 
  top = min((((filterSamples * cutOffT) / 100) + 1  ), (filterSamples - 1));   // the + 1 is to make up for asymmetry caused by integer rounding
  k = 0;
  total = 0;
